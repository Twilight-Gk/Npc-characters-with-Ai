...
public class State
{
    ...
    public  bool OnAttack()
    {
        ...
        if(direction.magnitude < sneakdistance)
        {
            return true;
        }
        return false;
...
/*public class Runtosafety : State
{
    
    public Runtosafety(GameObject _npc, NavMeshAgent _agent, Animator _anim, Transform _player)
                : base(_npc, _agent, _anim, _player)
    {
        name = STATE.RUNTOSAFETY;
    ...
    public override void Update()
    {
        ...
        float distance = Vector3.Distance(npc.transform.position, GameObject.FindGameObjectWithTag("Safe").transform.position);
        if (distance < 10f)
        {
